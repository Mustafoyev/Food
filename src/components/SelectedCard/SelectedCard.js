...
export const SelectedCard = () => {
	...
	const handleDelItem = (id) => {
		setDish([...dish].filter((el) => el.id !== id));
	};

	return (
		<div className='selected-card'>
			{dish.length ? (
				<ul className='selected-card__list'>
					{dish.map((el) => (
						<li className='selected-card__item' key={el.id}>
							<div className='selected-card__top'>
								<img
									className='selected-card__item-img'
									src={`http://localhost:5000/${el.image}`}
									alt={el.name}
									width={40}
									height={40}
								/>
								<div className='selected-card__top-wrapper'>
									<h3 className='selected-card__title'>{el.name}</h3>
									<span className='selected-card__top-price'>{el.price}</span>
								</div>
								<span className='selected-card__top-total'>{countDish}</span>
								<span className='selected-card__top-totalprice'>
									{el.price}
								</span>
							</div>
							<div className='selected-card__bottom'>
								...
								<button
									...
									className='selected-card__bottom-btn'>
									<svg
										width='20'
										height='20'
										viewBox='0 0 20 20'
